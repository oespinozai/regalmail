...
import { normalizeEmail } from "./monitor-normalize.js";
 ...
 */
export async function startEmailMonitor(options) {
...
async function runIdleLoop(imapAccount, accounts, ownAddresses, abortSignal, onEmail, log) {
 ...
 */
async function processUnseen(client, accounts, ownAddresses, onEmail, log) {
    ...
    for (const uid of uids) {
        try {
            ...
            // Anti-loop checks
            const skipReason = shouldSkipReply(email, ownAddresses, matchedAccount.config.rateLimitPerHour);
            if (skipReason) {
