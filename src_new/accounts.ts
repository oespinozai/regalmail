 ...
 */
function resolvePassword(pass: string | { cmd: string } | undefined): string {
  if (!pass) return "";
 ...
 */
export function listEmailAccountIds(cfg: Record<string, unknown>): string[] {
  const emailCfg = (cfg as any)?.channels?.email;
  if (!emailCfg) return [];
...
export function resolveEmailAccount(
  ...
  const accountCfg: EmailAccountConfig =
 ...
 */
export function matchAccountByRecipient(
  accounts: ResolvedEmailAccount[],
  recipients: string[],
): ResolvedEmailAccount | undefined {
